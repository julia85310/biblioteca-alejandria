...
export default function Header({ubiHeader}){
    ...
    return <header className={`relative flex flex-row items-center lg:items-end px-6 pt-6 lg:px-8 lg:pt-6 justify-between pb-6 ${user?.admin ? "bg-[var(--aliceBlue)]" : "bg-[var(--seashell)]"}`}>
        <img src="../logo.png" className="w-24 lg:w-28"/> 
        ...
        <nav className={`${menuHidden ? 'hidden' : 'flex'} lg:border-0 pr-12 border-[var(--cafeNoir)] border-2 rounded-xl z-50 ${user?.admin ? "bg-[var(--aliceBlue)]" : "bg-[var(--seashell)]"} pl-12 py-8 top-6 right-4 pr-4 text-center lg:text-base text-sm gap-6 lg:relative absolute z-20 lg:static lg:flex flex-col lg:flex-row justify-end items-end lg:gap-18 lg:px-12 lg:pb-4`}>
            ...
            <p className={`text-xs w-full underline-animate ${ubiHeader === "Home" ? "active" : ""} ${animating === "Home" ? "animate" : ""}`} onClick={()=>handleClickNav("Home", ubiHeader)}>Conócenos</p>
            <p className={`text-xs w-full underline-animate ${ubiHeader === "Catalogo" ? "active" : ""} ${animating === "Catalogo" ? "animate" : ""}`} onClick={()=>handleClickNav("Catalogo", ubiHeader)}>Catálogo</p>
            <p className={`text-xs w-full underline-animate ${ubiHeader === "Perfil" ? "active" : ""} ${animating === "Perfil" ? "animate" : ""}`} onClick={()=>handleClickNav("Perfil", ubiHeader)}>{user?.admin ? "Panel de Administración" : "Mi perfil"}</p>
        </nav>
    </header>
}
