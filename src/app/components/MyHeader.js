...
export default function MyHeader({ubiHeader}){
    ...
    return <header 
        className={`relative flex flex-row items-center lg:items-end px-6 pt-6 lg:px-8 lg:pt-6 justify-between pb-6 ${fondo}`}>
        ...
        <nav className={`${menuHidden ? 'hidden' : 'flex'} lg:border-0 pr-12 border-[var(--cafeNoir)] border-2 rounded-xl z-50 ${fondo} pl-12 py-8 top-6 right-4 pr-4 text-center text-sm gap-6 lg:relative absolute z-20 lg:static lg:flex flex-col lg:flex-row justify-end items-end lg:gap-18 lg:px-12 lg:pb-4`}>
